...
def calc_metrics(y_true, y_pred, pred_score):
    ...
    y_true_bi=F.one_hot(y_true.to(torch.int64),num_classes=71)
    if task !='1':
        idx=np.argwhere(np.all(np.array(y_true_bi)[...,:]==0,axis=0))
        y_true_bi=np.delete(y_true_bi,idx,axis=1)
        pred_score=np.delete(pred_score,idx,axis=1)
...
def Train(batch_size=2000,n_epoch=200):
    ...
    save_dir = 'log\\multi_class\\'
    logging_config(folder=save_dir, name='mc_log_task{}'.format(task), no_console=False)
...
if __name__=='__main__':
    ...
    filename = ['Multi-Class-data2\Multi-Class Dataset.csv',
